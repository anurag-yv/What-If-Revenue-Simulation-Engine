...
function calculateWeeklyRevenue(
  ...
  for (let i = 0; i < q3Deals.length; i++) {
    ...
    expectedCloseDate.setDate(
      expectedCloseDate.getDate() + baseCycleDays + cycleShift
    );

    //calculate week index
    const diffDays = Math.floor(
      (expectedCloseDate.getTime() - q3Start.getTime()) / (1000 * 60 * 60 * 24)
    );
...
app.post('/api/simulate', (req, res) => {
  ...
  const baselineResult = calculateWeeklyRevenue(
    q3PipelineDeals,
    baseConversion,
    1,
    0,
  ...
  const scenarioResult = calculateWeeklyRevenue(
    q3PipelineDeals,
    newConversion,
    newSizeMultiplier,
    salesCycleChange,
  ...
  if (conversionChange !== 0) {
    driversList.push(
      conversionChange > 0 ? "Increase in conversion rate" : "Decrease in conversion rate"
    );
  }

  if (dealSizeChange !== 0) {
    driversList.push(
      dealSizeChange > 0 ? "Increase in average deal size" : "Decrease in average deal size"
    );
  }

  if (salesCycleChange !== 0) {
    driversList.push(
      salesCycleChange < 0 ? "Decrease in sales cycle" : "Increase in sales cycle"
    );
  }
